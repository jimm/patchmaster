...
This is a Ruby version of PatchMaster, realtime MIDI performance software.
It's a rewrite of [http://jimmenard.com/projects/keymaster/].
...
* start bytes sent
* PC sent
* connection attached to input device

For each input, when data seen
* data sent through connection to output

== Filters

Filter gets two args: device and MIDI bytes. It gets connection in case it
wants to modify it connection (change xpose, zone, --- even the filter)
