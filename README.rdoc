...
At all times, when MIDI data comes in from an instrument two things happen:
...
* The data is then sent next 

While a patch is running (it is the current patch), each 
 connection is given
the incoming bytes to process. incoming data on each
of its connection's inputs is sent through one or more connections to the proper outputs. Each
connection can filter and modify the data on the way:

* The data is then processed by each conne
