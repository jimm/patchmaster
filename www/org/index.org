...
#+begin_src sh
  patchmaster [-n] [-i] [-w] [-p port] [-d] [patchmaster_file]
#+end_src
...
To run PatchMaster from within an IRB session use -i. Reads ./.patchmasterrc
if it exists, $HOME/.patchmasterrc if not. See [[file:irb.org][IRB mode]] for details on the
