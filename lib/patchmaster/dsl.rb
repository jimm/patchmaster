module PM
  ...
  class DSL
    ...
    def input(port_num_or_name, sym, name = nil)
      ...
      input = InputInstrument.new(sym, name, port_num_or_name, @pm.use_midi?)
      @inputs[sym] = input
      @pm.inputs << input
    rescue StandardError => e
      raise "input: error creating input instrument \"#{name || sym}\" on input port #{port_num_or_name}: #{e}"
    end
    alias inp input
    ...
    def output(port_num_or_name, sym, name = nil)
      ...
      output = OutputInstrument.new(sym, name, port_num_or_name, @pm.use_midi?)
      @outputs[sym] = output
      @pm.outputs << output
    rescue StandardError => e
      raise "output: error creating output instrument \"#{name || sym}\" on output port #{port_num_or_name}: #{e}"
    end
    alias out output
    alias outp output
