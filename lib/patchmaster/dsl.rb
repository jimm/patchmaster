...
module PM
  ...
  class DSL
    ...
    def input(port_num, sym, name = nil)
      ...
      raise "input: error creating input instrument \"#{name || sym}\" on input port #{port_num}: #{e}"
    end
    alias inp input
    ...
    def output(port_num, sym, name = nil)
      ...
      raise "output: error creating output instrument \"#{name || sym}\" on output port #{port_num}: #{e}"
    end
    alias out output
    alias outp output
    ...
    def message_key(key_or_sym, name)
      if name.is_a?(Symbol)
        ...
        warn 'WARNING: the arguments to message_key are now key first, then name.'
        warn "I will use #{name} as the name and #{key_or_sym} as the key for now."
        warn 'Please swap them for future compatability.'
      end
    ...
    def zone(start_or_range = nil, stop = nil)
      @conn.zone = if start_or_range.is_a? Range
                     start_or_range
    ...
    def save_messages(f)
      @pm.messages.each_value do |(correct_case_name, msg)|
    ...
    def save_code_keys(f)
      @pm.code_bindings.each_value do |code_key|
    ...
    def save_connection(f, conn)
      ...
      if conn.zone
        f.puts "      zone #{conn.note_num_to_name(conn.zone.begin)}, #{conn.note_num_to_name(conn.zone.end)}"
      end
    ...
    def to_binding_key(key_or_sym)
      return unless key_or_sym.is_a?(Symbol) && PM::Main::FUNCTION_KEY_SYMBOLS[key_or_sym]

      PM::Main::FUNCTION_KEY_SYMBOLS[key_or_sym]
    end
    ...
    def read_block_text(name, containers, contents)
      ...
      contents.each_line do |line|
        if line =~ /^(\s*)#{name}\s*.*?(({|do|->\s*{|lambda\s*{)(.*))/
          block_indentation = ::Regexp.last_match(1)
          ...
          if indentation.length <= block_indentation.length
            chunk.text << line if text =~ /^#{block_end_token}/
            in_block = false
          else
            chunk.text << line
          end
