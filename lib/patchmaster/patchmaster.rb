...
module PM
  ...
  class PatchMaster < SimpleDelegator
    ...
    attr_reader :inputs, :outputs, :all_songs, :song_lists, :messages, :message_bindings, :code_bindings, :loaded_file
    attr_accessor :use_midi, :gui
    alias use_midi? use_midi
    ...
    def initialize
      ...
      @debug_file = File.open(DEBUG_FILE, 'a') if $DEBUG
    ...
    def save(file)
      ...
      raise("error saving #{file}: #{e}" + caller.join("\n"))
    end
    ...
    def panic(individual_notes = false)
      ...
      @outputs.each do |out|
        ...
        MIDI_CHANNELS.times do |chan|
          ...
          buf += (0..127).collect { |note| [NOTE_OFF + chan, note, 0] }.flatten if individual_notes
        end
        out.midi_out(buf)
      end
    end
