...
module PM
  ...
  class PatchMaster < SimpleDelegator
    ...
    attr_reader :inputs, :outputs, :all_songs, :song_lists, :messages,
                :message_bindings, :code_bindings
    attr_accessor :gui
    ...
    def use_midi? = @use_midi

    def use_midi=(val)
      @use_midi = val
      return unless @use_midi
    ...
    def run
      if @gui
        %i(run start).each { @gui.send(_1) if @gui.respond_to?(_1) }
      else
        start(true)
        @inputs.each(&:stop)
        stop
    ...
    def panic(individual_notes = false)
      ...
      @outputs.each do |out|
        ...
        MIDI_CHANNELS.times do |chan|
          ...
          buf += (0..127).map { |note| [NOTE_OFF + chan, note, 0] } if individual_notes
        end
        out.midi_out(buf)
      end
    end
