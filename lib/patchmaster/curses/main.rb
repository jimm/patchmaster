        ...
        loop do
          begin
            ...
            when 'g'
              ...
              @pm.goto_song(name) if name.length.positive?
            when 't'
              name = PromptWindow.new('Go To Song List', 'Go to Song List:').gets
              @pm.goto_song_list(name) if name.length.positive?
            when 'e'
              close_screen
              file = @pm.loaded_file || PromptWindow.new('Edit', 'Edit file:').gets
              edit(file) if file.length.positive?
            when 'r'
              load(@pm.loaded_file) if @pm.loaded_file&.length&.positive?
            when 'h', '?'
              help
