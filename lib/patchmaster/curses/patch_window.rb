...
module PM
  class PatchWindow < PmWindow
    ...
    def draw_connection(connection)
      ...
      str << " #{connection.input_chan ? format('%2d', connection.input_chan + 1) : '  '} |"
      str << " #{'%16s' % connection.output.name}"
      str << " #{format('%2d', connection.output_chan + 1)} |"
      ...
      str << if connection.xpose && connection.xpose != 0
               "   #{connection.xpose.negative? ? '' : ' '}#{format('%2d', connection.xpose.to_i)} |"
             else
               '       |'
             end
